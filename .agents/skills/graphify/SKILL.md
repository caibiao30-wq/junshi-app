---
name: graphify
description: "Use when the user explicitly asks to build, update, query, explain, or export a graphify knowledge graph for code, documents, papers, images, or video. Do not trigger for ordinary codebase questions or book editing; those should use the repository's normal routing first. This skill can install tools, access external content, and write graphify-out artifacts, so confirm scope before any side effect."
---
